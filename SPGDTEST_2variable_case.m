...
dJ(1,1)= 1;
dUsave(1,:,1) = abs(After_U-Before_U);
...
for ii = 1:Iterration_Count
    ...
    Gamma(1,ii) = (1-(ii/1000)+(100/(ii^1.2)))/max(Target_Intensity_Sum(jj,:));
    Gamma(isnan(Gamma)) = 1;
    weight=Gamma .* (dJ(1,ii+1))./(Variance_dU);
    dUsave(1,:,ii) = dU;
    J_prime = (weight.*(dU)); 
...
% subplot(2,2,3);
% hold on;
% plot((Usave(1,1,:)./(2*pi)),'r');
% plot((Usave(1,2,:)./(2*pi)),'b');
% hold off;
% title('각 beam의 위상'); 
% subplot(2,2,4);
% hold on;
% plot((dUsave(1,1,:)./(2*pi)),'r');
% plot((dUsave(1,2,:)./(2*pi)),'b');
% hold off;
