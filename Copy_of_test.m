...
%     u0=uu;
    ausave(:,:,ii+1) = au;
    au_1 = padarray(expand(au),[128 128],0,'both');
    au_2 = padarray(expand(-au),[128 128],0,'both');

    u0=propTF(uu.*exp(-1i*au_1),L1,lambda,2*z);
%     [u0]=focus(u0,L1,lambda,zf);
    u0=propTF(u0,L1,lambda,2*z);
    u00=propTF(uu.*exp(-1i*au_2),L1,lambda,2*z);
    u00=propTF(u00,L1,lambda,2*z);
    Intensity1 = abs(u0).^2;
    Intensity2 = abs(u00).^2;
    Isave(:,:,ii) = Intensity1;
    
    JJ(1,ii+1) = sum(sum((Intensity1)));
    J1(1,ii+1) = chk_J((Intensity1),S);
    J2(1,ii+1) = chk_J((Intensity2),S);
% % % % %     if (J(1,maxJ)<=J(1,ii+1))
% % % % %         maxJ = ii+1;
% % % % %     end
    
    Jratio(1,ii+1)=((J1(1,ii+1)+J2(1,ii+1))/(2*JJ(1,ii+1)))*100;
%     J(1,ii+1) = J1(1,ii+1)/JJ(1,ii+1);
    W(1,ii+1)=((J1(1,ii+1) - J2(1,ii+1)));% /mean(J)
...
%     diffU(isnan(diffU)) = 0;
% % % % %     if (maxJ == ii+1)
% % % % %         dU = ausave(:,:,maxJ) - ausave(:,:,ii);
% % % % %         weight = J(1,maxJ) - J(1,ii);
% % % % %     else
% % % % %         dU = ausave(:,:,maxJ) - ausave(:,:,ii+1);
% % % % %         weight = J(1,maxJ) - J(1,ii+1);
% % % % %     end
%     if sum(sum(BB))<=0.01
%         break
%     end
...
%     WM = WM./(max(max(WM))/(2*pi));
    ...
    MaxI(1,ii+1)=max(max((Intensity1)));
    MinI(1,ii+1)=min(min((Intensity1)));
    bu = au;
%     bu = rem(bu, (2*pi));
%     aum = (au + weight);%2
    au = (au - WM);%2
